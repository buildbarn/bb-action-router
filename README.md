...
There are two main modes of operation: inline and bind_mount. In inline mode the action router
will merge the contents of the container with the action's input root and the helper will
`chroot` into the action's input root. In bind_mount mode the action router only rewrites the
command line and the container pull is performed by `bb_docker_root_fetcher`, an new
service that needs to run alongside each worker process.
...
Which approach to pick depends on the specifics of your deployment:
 ...
 - bind_mount is more complex to set up as it requires an extra service and will result in more load on the registry
...
If you'd like to use inline mode you should look at `bb_docker_action_router` and 
`bb_chroot_helper_privileged`, while for bind_mount mode you should use `bb_docker_action_router`,
`bb_chroot_helper` and `bb_docker_root_fetcher`.